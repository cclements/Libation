...
| S5 — Processing and The Decanter | Complete; source, automated, visual, and scoped interaction review Passed | The existing `ProcessQueueViewModel` remains the sole queue/execution owner. One stable grouped projection now supplies truthful Active/Waiting/Completed/failed-only Failed counts, aggregate progress, typed processing stages, delegated performance/log/recovery commands, title-specific accessibility state, and the one shell-reparented Decanter. The approved focused S5 packet passes 5/5. The unchanged full Release solution gate passes 1,785 total with 1,762 passed, 23 expected platform/secret-store skips, and 0 failed. Exact-current Release and Debug Avalonia builds pass with 0 warnings/errors. Sixteen canonical and four expanded-failure exact-size isolated macOS captures cover both profiles and binding sizes; six same-input reference sheets and an independent original-density audit pass with no S5-owned P0/P1/P2 finding. Exact-current safe interaction passes Performance, the queue log, full profile-chooser traversal, failed-row Copy details, and failed-row Open log without starting processing. Evidence: outer `runtime-audit-2026-09-02/S5/` and nested `design-qa.md`. | Local macOS evidence does not prove VoiceOver or other assistive technology, live retry/cancel outcomes, keyboard traversal, 200% logical scaling, runtime High Contrast/reduced motion, Windows/Linux, packaging, notarization, publication, or release. S6 remains unopened. |
