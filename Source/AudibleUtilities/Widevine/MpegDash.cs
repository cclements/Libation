...
public class MpegDash
{
	...
	public bool TryGetContent(
		...
		=> TryGetContent(baseUri, protectionSystemId, expectedCodec: null, out fileUri, out pssh);
		...
		[NotNullWhen(true)] out PsshBox? pssh)
	{
		...
		foreach (var element in hierarchy)
		{
			...
			{
				if (TryParsePssh(protection.Element(cenc + "pssh"), protectionSystemId, out pssh))
					return true;
			}
		}
		pssh = null;
		...
		[NotNullWhen(true)] out PsshBox? pssh)
	{
		...
		if (psshElement?.Value.Trim() is not { Length: > 0 } psshString
			|| psshElement.Parent?.Attribute("schemeIdUri")?.Value is not string uuid
			|| !uuid.Equals(UuidPreamble + protectionSystemId, StringComparison.OrdinalIgnoreCase))
			return false;
