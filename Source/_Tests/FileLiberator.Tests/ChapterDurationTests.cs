...
public class ChapterDurationTests
{
	...
	private static Chapter Chapter(string title, long start, long length)
		=> new() { Title = title, StartOffsetMs = start, LengthMs = length, StartOffsetSec = start / 1000 };

	private static DownloadOptions Options(long intro, long outro, params Chapter[] chapters)
	{
